#!/bin/bash
# Build the dungeon crawler, package, and launch in TiEmu
set -eu
...
echo "=== Step 1: Build dungeon crawler in Docker ==="
docker run --platform linux/amd64 --rm \
  ...
  gcc4ti-builder bash -c '
    ...
    cd /build/repo
    echo "=== Compiling fatd_dungeon ==="
    cd src && tigcc -O3 -fomit-frame-pointer -Wall -W -Wwrite-strings fatd_dungeon.c -pack fatddung && cd ..
    echo "=== Build complete ==="
    mv src/fatddung.89y output/ 2>/dev/null || true
    mv src/fatd_dungeon.89y output/ 2>/dev/null || true
    mv src/fatd_dungeon.89z output/ 2>/dev/null || true
    ls -la output/fatddung.89y 2>/dev/null || echo "Looking for output..."
    ls -la output/*.89y output/*.89z 2>/dev/null || true
  '

DEMO_FILE=""
if [[ -f "$REPO_DIR/output/fatddung.89y" ]]; then
  DEMO_FILE="$REPO_DIR/output/fatddung.89y"
fi
...
python3 "$TOOLS_DIR/make89g.py" "$REPO_DIR/output/dungeon.89g" \
...
DYLD_LIBRARY_PATH="$TIEMU_DYLD_LIBRARY_PATH" "$TIEMU_BIN" "$ROM" \
  "-send=$REPO_DIR/output/dungeon.89g" \
  "-exec=fatload()" \
  "-hwprot=0" &
echo "TiEmu PID: $!"
echo ""
