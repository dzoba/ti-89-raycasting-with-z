...
* file name:      gentransscalers.c
* initial date:   19/03/2001
* author:         thomas.nussbaumer@gmx.net
*
* description:    generates 68kASM scaling routines for vertical texture
*                 scaling using an additionally transparency mask
*
...
*
* $Id: gentransscalers.c,v 1.4 2002/05/02 09:28:36 tnussb Exp $
*
******************************************************************************/
#include <stdio.h>
#include <stdlib.h>
...
//-----------------------------------------------------------------------------
#define END_HEIGHT              120         // >= VISIBLE_HEIGHT and even
#define DOUBLE_STEP_HEIGHT      30          // >= START_HEIGHT
...
void WriteEntry(int height, int lsr) {
    ...
    printf("jmp __tr_scale%03d(%%pc)\n",height);                                  // jump to scaler code

    //--------------------------------------------------------------------
    // entry for second half ...
    //--------------------------------------------------------------------
    // shift texture if necessary
    if (lsr>0 && lsr<9)       printf("lsr.l #%d,%%d0;lsr.l #%d,%%d3\n",lsr,lsr);
    else if (lsr>8 && lsr<32) printf("ERROR: INVALID SHIFT\n"); //printf("lsr.l #8,%%d0;lsr.l #%d,%%d0;lsr.l #8,%%d3;lsr.l #%d,%%d3\n",lsr-8,lsr-8);
    else                      printf("nop;nop\n");
...
* d3 .... transparent mask of upper part
*
* -----------------------------------------------------------------------------
...
* d3 .... transparent mask of lower part
*
*
* =============================================================================
* jumptable format
...
void GenerateScalers() {
    ...
    //-------------------------------------------------------------------------
    printf("#define TRANS_CLIP_HEIGHT %d // max. height usable without crashing\n",END_HEIGHT-2); // -2 to make sure not to run out
    ...
    printf("INTERNAL_REFERENCED unsigned short sprite_delta[%d/2]={\n",END_HEIGHT-START_HEIGHT+2);
    for (i=START_HEIGHT;i<=END_HEIGHT;i+=2) {
         printf("(63<<8)/%d",i);

         if (i<END_HEIGHT) {
             printf(",");
             if ((i-START_HEIGHT)%10 == 8) printf("\n");
         }
...
//#############################################################################
//
// $Log: gentransscalers.c,v $
// Revision 1.4  2002/05/02 09:28:36  tnussb
// sprite delta table modified to use 63 instead of 64
//
// Revision 1.3  2002/04/19 08:39:47  tnussb
// missing shift-right by one added
//
// Revision 1.2  2002/03/28 12:26:38  tnussb
// timestamp in generated output removed so CVS will not get "confused" anymore
//
// Revision 1.1  2002/03/01 11:06:03  tnussb
